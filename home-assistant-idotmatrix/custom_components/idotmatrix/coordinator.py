...
class IDotMatrixDataUpdateCoordinator(DataUpdateCoordinator):
    """Class to manage fetching data from the API."""    def __init__(self, hass: HomeAssistant, entry: ConfigEntry) -> None:
        """Initialize."""
        self.entry = entry
        self.hass = hass
    ...
    async def async_disconnect(self) -> None:
        ...
        if self._device and self._connected:
            try:
                ...
                self._connected = False
                self._device = None    async def _async_update_data(self) -> dict[str, Any]:
