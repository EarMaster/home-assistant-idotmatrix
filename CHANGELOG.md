...
### Fixed
- GIF uploads (**Image: File** with `.gif`, **Image: Icon & Message**) no longer leave the screen black; calling `image.set_mode(EnableDIY)` before a GIF upload was putting the device into static-image mode, causing it to silently ignore the subsequent GIF packets (command byte `1`) and leave a black screen
